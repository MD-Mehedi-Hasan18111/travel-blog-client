          ...
          <div>
            {allComments?.map((comment) => (
              <div className="my-4">
                ...
                {blog?.email == user?.email || comment?.commentAuthor == user?.displayName ? <p onClick={() => handleDeleteComment(comment._id)} className="m-0" style={{cursor: "pointer", color: "#0B5ED7"}}>Delete Comment</p>: ""}
              </div>
            ))}
          </div>
        )}
