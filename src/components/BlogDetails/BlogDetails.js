...
const BlogDetails = () => {
  ...
  useEffect(() => {
    fetch(`https://travel-agency-me.herokuapp.com/comment/${id}`)
      .then((res) => res.json())
      .then((data) => setAllComments(data));
  }, [allComments]);

  useEffect(() => {
    fetch(`https://travel-agency-me.herokuapp.com/blog/${id}`)
      .then((res) => res.json())
  ...
  const handleSubmit = (e) => {
    ...
    fetch("https://travel-agency-me.herokuapp.com/comment", {
      method: "POST",
      headers: {
          ...
          <div>
            {allComments?.map((comment) => (
              <div className="my-4">
                ...
                {blog?.email == user?.email && user?.displayName == comment?.commentAuthor ? <p onClick={() => handleDeleteComment(comment._id)} className="m-0" style={{cursor: "pointer", color: "#0B5ED7"}}>Delete Comment</p>: ""}
              </div>
            ))}
          </div>
        )}
